...
public class DBConnector {
    ...
    public Future<Void> updateQueryWithParam(String query, JsonArray params) throws NoSuchElementException {
        ...
        Future<Void> response = Future.future();
        ...
        client.updateWithParams(query, params, result -> {
            if (result.failed()) {
                ...
                if (result.result().getUpdated() == 0) {
                    response.fail(new NoSuchElementException("Unknown service "));
                } else {
                    response.complete();
                }
            }
